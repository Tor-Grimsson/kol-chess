...
- 2026-08-26 — [KOL bump ×3 (chess 0.7.1 · component 0.79.0 · theme 0.58.1 …), mobile scan + fixes, Find-games recipes, first lobby round-trip](session-log/2026-08-26-kol-bump-mobile-scan-find-games.md)
...
- **Eight pages in kol-shell's `AppShell`** (2026-08-31 Play & Insight; NavRail migration 2026-08-30 underneath): flat 48px `NavRail` — Overview · Board · Play · Database · Statistics · Insights · The bot, Settings pinned below the rule. `/` landing · `/analysis` board · **`/play`** a real game surface (new game→options→Start on a DS `ShellDrawer`, clocks flanking the board, lichess/chess.com time-control ladder, notation/resign/takeback/promotion, extras toggle default OFF) · `/database` Query/Browse/Learn (+ opening recipes O1–O6) · `/stats` (+ King's Gambit / Dutch / French pillar cards) · **`/insights`** diagnosis, opening autopsy, suggestions, sampled engine pass · **`/bot`** how the model is made · `/settings`. **Law:** a local build of anything the DS ships is a defect — search kol-component/kol-shell/kol-framework and the estate first.
- **The bot is a position book, not a model.** `scripts/build-style-book.mjs` walks the 106 CDN PGN files once (~71 MB, cached in `_tmp/pgn-cache`) and emits JSON keyed by `epdOf(fen)` → move → `{n, s, y, b, o, t}`. Three layers: STYLE (whole corpus, never filtered) · PERIOD (year/band/opponent-band/time-class re-weight, blended so a thin slice degrades to style instead of going silent) · STRENGTH (sharpness + `UCI_Elo` out of book). Same pipeline builds **nine masters** (13,024 games, pgnmentor — statistics derived, PGN not redistributed). Artifacts live in `public/books/` and are **fetched, not imported** — as modules they compiled to multi-MB JS chunks.
...
- **Stack:** React 19 · Vite 8 · Tailwind CSS v4 · pnpm · **KOL DS** (`kol-theme@0.111.0` / `kol-component@0.143.0` / `kol-icons@0.25.0` / `kol-framework@0.36.0` / `kol-shell@0.30.0` / `kol-chess@0.8.0` / `kol-dashboards@0.2.3`, all latest 2026-08-30) · `gsap` (NavRail's peer) · `chess.js` · `stockfish` · `@duckdb/duckdb-wasm` · CodeMirror (sql).
...
**Three tickets out, one owing work back.** `ChessBoardInputAndVariantSeam` (kol-ds-ui 🔵) carries a 📌: drag + square a11y are **prototyped and working in the linked kol-ds-ui tree, not in this repo** — on ship, unlink, bump kol-chess, drop the local `touch-action` wrapper in `PlayPage.jsx`. Also open: `DashCardBadgePropIsDead` (🔵) and dotfiles' `lobby-watch-exit-when-idle` (🔵). **Blocked on a ruling:** the `dests` seam and chess.js → chessops — `../llm-plan/03-chessops-migration.md` is scoped, not started. Unowned decisions: the 19 MB of artifacts (repo vs B2 CDN) and whether the local `Clock` graduates into the chess package.
...
- **THE CHESS PACKAGE IS CURRENTLY SYMLINKED (2026-08-31).** `node scripts/link-chess-package.mjs` points `@kolkrabbi/kol-chess` at `../kol-ds-ui/packages/chess` so board work needs no publish. `package.json` is untouched, so a clone and CI still install the published copy — but `pnpm install` silently reverts the link, and with it the drag/a11y prototype. `--status` says which state you are in. **`vite.config.js`'s `resolve.dedupe` is load-bearing for this**: a symlinked package resolves its own imports from the DS monorepo, which gave a second React and kol-component's unpublished source. `alias` is the wrong fix — it flattens the exports map and 500s subpath imports like `kol-component/utilities/motion`.
- **Verified at logic level only, not through the UI** — the promotion picker (chess.js contract proven in node; no test game reached the 8th rank) and clock flagging (unit-tested; never watched hit zero live). Six of the nine masters have never played a full game, only statistical checks of their opening moves.
