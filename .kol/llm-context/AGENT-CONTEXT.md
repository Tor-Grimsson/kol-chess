...
**Last updated:**
- 2026-09-01 — [Mobile review round 2: eight items — presets lg/primary, sheet headline cut, insight cards off the 496px row floor, master portraits on the roster; three DS tickets filed AND shipped (drawer mirrored right, one close idiom, `.dash-lede`), one caught minting a duplicate class and re-shipped](session-log/2026-09-01-mobile-review-round-2-portraits.md)
- 2026-09-01 — [Mobile field review: font-folder case bug fixed live, favicon style leak found + hardened upstream, PlayerBar, confirms, first-person voice; 3 DS tickets filed AND closed same day](session-log/2026-09-01-mobile-field-review-fixes.md)
...
- **Eight pages in kol-shell's `AppShell`** (2026-08-31 Play & Insight; NavRail migration 2026-08-30 underneath): flat 48px `NavRail` — Overview · Board · Play · Database · Statistics · Insights · The bot, Settings pinned below the rule. `/` landing · `/analysis` board · **`/play`** a LOBBY until a game exists (2026-09-01) — big New game, the eleven opponents as DS `ContentRow variant="roster"` rows carrying 120×120 webp portraits (`public/masters/`, Wikimedia for the masters, the website's B2 studio portrait for `me`; `engine` falls back to initials), and **no board and no book fetched on arrival**; New game opens a `FullscreenOverlay` sheet with a grouped time-control preset grid, then the board mounts with clocks flanking it · `/database` Query/Browse/Learn (+ opening recipes O1–O6) · `/stats` (+ King's Gambit / Dutch / French pillar cards) · **`/insights`** diagnosis, opening autopsy, suggestions, sampled engine pass · **`/bot`** how the model is made · `/settings`. **Law:** a local build of anything the DS ships is a defect — search kol-component/kol-shell/kol-framework and the estate first.
...
- **Stack:** React 19 · Vite 8 · Tailwind CSS v4 · pnpm · **KOL DS** (`kol-theme@0.120.1` / `kol-component@0.152.0` / `kol-icons@0.25.0` / `kol-framework@0.36.0` / `kol-shell@0.34.0` / `kol-chess@0.10.0` / `kol-dashboards@0.4.1`, all latest 2026-09-01 night) · **`chessops`** (the rules engine; `chess.js` is a devDependency kept only for the differential test) · `onnxruntime-web` · `stockfish` · `gsap` · `@duckdb/duckdb-wasm` · CodeMirror (sql). **Python lives in `train/`** — a 3.9 venv, torch 2.2.2 CPU-only (this iMac's Radeon is unreachable by PyTorch).
...
**NOTHING OWED IN CODE (2026-09-01 night).** All eleven receipts 🟢 with remainders executed — round 2's three (drawer side, overlay close idiom, card-subtitle leading) filed and closed the same evening, bumps done. **Every fix from both rounds is deploy-pending: nothing has been seen on the phone since round 2 began.** The 390 sweep of all eight routes in both themes is owed after the next deploy — now also covering the mirrored drawer (+ the DS's flagged frame: a long drawer title reaching under the top-right X), the new-game sheet, the roster portraits and the insight cards. Unowned decisions remain: the artifacts' home (19 MB in `public/books/` + a 9.4 MB model, repo vs B2 CDN) and whether the local `Clock`/`PlayerBar` graduates into the chess package.

## Active known issues

- **`GridCard span` is a row FLOOR, not a hint.** `.dash-grid` rows are `minmax(240px, auto)`, so `2x2` reserves 496px whatever the content is, and the pack's mobile clamp rewrites `grid-column` for oversized spans but never `grid-row`. `/insights`'s table cards are `2x1` for that reason (2026-09-01); the `/stats` CHART cards keep `2x2` because a chart needs a box. A table card that gets a 2-row span will show a screenful of dead space under it on a phone.
