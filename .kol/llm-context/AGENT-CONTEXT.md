...
- **Stack:** React 19 · Vite 8 · Tailwind CSS v4 · pnpm · **KOL DS** (`kol-theme@0.119.0` / `kol-component@0.148.0` / `kol-icons@0.25.0` / `kol-framework@0.36.0` / `kol-shell@0.32.0` / `kol-chess@0.10.0` / `kol-dashboards@0.3.0`, all latest 2026-09-01 eve) · **`chessops`** (the rules engine; `chess.js` is a devDependency kept only for the differential test) · `onnxruntime-web` · `stockfish` · `gsap` · `@duckdb/duckdb-wasm` · CodeMirror (sql). **Python lives in `train/`** — a 3.9 venv, torch 2.2.2 CPU-only (this iMac's Radeon is unreachable by PyTorch).
...
**NOTHING OWED (2026-09-01 eve).** All eight receipts 🟢 with remainders executed — the field-review three (heading font var, shell page pad, logomark sanitiser) filed and closed the same evening, bumps done. **Unpushed:** everything after `chess-0006` (consumer fixes, bumps, voice sweep) — the 390 live sweep of all routes is owed after the user's next deploy. Unowned decisions remain: the artifacts' home (19 MB in `public/books/` + a 9.4 MB model, repo vs B2 CDN) and whether the local `Clock`/`PlayerBar` graduates into the chess package.
