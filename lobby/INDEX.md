...
|---|---|---|---|---|
| 🟢 | [ShellDrawerOnRight](outbox/ShellDrawerOnRight.md) | **kol-ds-ui** — `~/dev/projects/kol-ds-ui/lobby/INDEX.md` | 🟢 `closed` · synced 2026-09-01 — kol-shell 0.34.0 + kol-theme 0.120.0 (drawer mirrored right, travel rule retired, scrim a button) | 📌 bumped same day; phone re-check (right-side drawer, X top-right, scrim tap, long-title frame) rides the next deploy |
| 🟢 | [FullscreenOverlayCloseIdiom](outbox/FullscreenOverlayCloseIdiom.md) | **kol-ds-ui** — `~/dev/projects/kol-ds-ui/lobby/INDEX.md` | 🟢 `closed` · synced 2026-09-01 — kol-component 0.152.0 (bare nav X) + kol-theme 0.120.0 (right: 0) | 📌 bumped same day; new-game-sheet phone re-check rides the next deploy |
| 🟢 | [DashDetailWrapsWithoutLeading](outbox/DashDetailWrapsWithoutLeading.md) | **kol-ds-ui** — `~/dev/projects/kol-ds-ui/lobby/INDEX.md` | 🟢 `closed` · synced 2026-09-01 — kol-theme 0.120.1 + kol-dashboards 0.4.1: the lede is `.dash-lede` (10→12, lh 1.5); 0.120.0/0.4.0 minted a duplicate `.dash-subtitle` over the existing sub-heading and are deprecated | 📌 bumped same day; `/insights` subtitle re-check at 390 rides the next deploy |
...
- **2026-09-01** — Round 2 of the phone review: filed three more into **kol-ds-ui** (🔵 ×3): `ShellDrawerOnRight` (the drawer's open X travels to 65% across a 390 viewport — user rules the right side), `FullscreenOverlayCloseIdiom` (two close-X treatments one tap apart + the overlay's 12px corner inset aligns with nothing), `DashDetailWrapsWithoutLeading` (`.dash-detail` is line-height 1 and CardHeader hands it a wrapping subtitle). Same round, fixed here as consumers: time presets lg/primary, sheet headline cut, Cancel primary, insight cards 2x2→2x1 (the 496px row floor was the "so much space" report), master portraits into the roster.
